...
module reflect {
    ...
    export function createLoader(): Loader {
        ...
        function processRootFile(filename: string): SourceFile {
        ...
        function processRootFileAsync(filename: string, callback: (err: Error, file: SourceFile) => void): void {
        ...
        function processExternalModule(moduleName: string, searchPath: string): Symbol {
            ...
            if (sourceFile) {
                ...
                errors.push(new Diagnostic(undefined, Diagnostics.File_0_is_not_an_external_module, sourceFile.filename));
                return;
            }

            errors.push(new Diagnostic(undefined, Diagnostics.Cannot_find_external_module_0, moduleName));
        }
        ...
        function bindPendingFilesAsync(callback: (err: Error) => void): void {
            ...
            function processFiles() {
                ...
                callback(errors.length > 0 ? createDiagnosticError(errors) : null);
            }
        }
        ...
        function processSourceFileAsync(filename: string, isDefaultLib: boolean, refFile: SourceFile, callback: (err: Error, result: SourceFile) => void): void {
            ...
            function handleCallback(err: Error, file: SourceFile, diagnostic?: DiagnosticMessage) {
                ...
                if (diagnostic) {
                    ...
                    return callback(createDiagnosticError(errors), null);
                }
                callback(null, file);
            }
        }
        ...
        function findSourceFileAsync(filename: string, isDefaultLib: boolean, refFile: SourceFile, callback: (err: Error, result: SourceFile) => void): void {
            ...
            readSourceFileAsync(filename, (err, file) => {
                ...
                if (err) {
                    errors.push(new Diagnostic(refFile, Diagnostics.Cannot_read_file_0_Colon_1, filename, err.message));
                    return callback(createDiagnosticError(errors), null);
                }
        ...
        function findSourceFile(filename: string, isDefaultLib: boolean, refFile?: SourceFile): SourceFile {
            ...
            if (hasProperty(filesByName, canonicalName)) {
                ...
                try {
                    ...
                    errors.push(new Diagnostic(refFile, Diagnostics.Cannot_read_file_0_Colon_1, filename, e.message));
                }
                if (file) {
                    seenNoDefaultLib = seenNoDefaultLib || file.noDefaultLib;
                    if (!options.noResolve) {
