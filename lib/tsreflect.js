...
(function (reflect) {
    (function (NodeKind) {
        ...
        NodeKind[NodeKind["Parameter"] = 26] = "Parameter";
        NodeKind[NodeKind["TypeParameter"] = 27] = "TypeParameter";
...
(function (reflect) {
    var Diagnostic = (function () {
        function Diagnostic(file, codeOrMessage, category, messageText) {
            this.file = file;
    ...
    var DiagnosticMessageChain = (function () {
        function DiagnosticMessageChain() {
        }
        DiagnosticMessageChain.prototype.flatten = function (file) {
            var code = this.code;
        ...
        DiagnosticMessageChain.chain = function (next, message) {
            var text = getLocaleSpecificMessage(message.key);
    ...
    reflect.DiagnosticMessageChain = DiagnosticMessageChain;
    ...
    reflect.Diagnostics = {
        ...
        Circular_definition_of_import_alias_0: { code: 1004, category: 1 /* Error */, key: "Circular definition of import alias '{0}'." },
        Cannot_find_name_0: { code: 1005, category: 1 /* Error */, key: "Cannot find name '{0}'." },
        Module_0_has_no_exported_member_1: { code: 1006, category: 1 /* Error */, key: "Module '{0}' has no exported member '{1}'." },
        File_0_is_not_an_external_module: { code: 1007, category: 1 /* Error */, key: "File '{0}' is not an external module." },
        Cannot_find_external_module_0: { code: 1008, category: 1 /* Error */, key: "Cannot find external module '{0}'." },
        Generic_type_0_requires_1_type_argument_s: { code: 1009, category: 1 /* Error */, key: "Generic type '{0}' requires {1} type argument(s)." },
        Type_0_is_not_generic: { code: 1010, category: 1 /* Error */, key: "Type '{0}' is not generic." },
    ...
    function createSourceFile(filename, text) {
        ...
        function scanTypeParameterDeclarations(nodes) {
            ...
            for (var i = 0, l = nodes.length; i < l; i++) {
                ...
                parameter.kind = 27 /* TypeParameter */;
        ...
        function scanFunctionDeclaration(node) {
            node.kind = 5 /* FunctionDeclaration */;
...
(function (reflect) {
    ...
    function printTypeErrors() {
        reflect.forEach(errors, function (x) {
            console.log(x);
        });
    }
    reflect.printTypeErrors = printTypeErrors;
    ...
    function createSymbol(flags, name) {
        return new reflect.Symbol(flags, name);
    }
    ...
    function resolveName(location, name, meaning) {
        ...
        function returnResolvedSymbol(s) {
            if (!s) {
                error(location, reflect.Diagnostics.Cannot_find_name_0, name);
            }
            return s;
        }
    ...
    function resolveEntityName(location, name, meaning) {
        ...
        if (name.length == 1) {
            ...
            if (!symbol) {
                error(location, reflect.Diagnostics.Module_0_has_no_exported_member_1, name.join('.'), rightName);
                return;
            }
        }
    ...
    function createType(flags) {
        var result = new reflect.Type(flags);
        result.id = typeCount++;
        return result;
    }
    ...
    function isOptionalProperty(propertySymbol) {
        ...
        return (propertySymbol.valueDeclaration.flags & 4 /* QuestionMark */) && propertySymbol.valueDeclaration.kind !== 26 /* Parameter */;
    }
    ...
    function getTypeOfPrototypeProperty(prototype) {
        ...
        return classType.typeParameters ? createTypeReference(classType, reflect.map(classType.typeParameters, function (_) {
            return anyType;
        })) : classType;
    }
    ...
    function getTypeOfVariableDeclaration(declaration) {
        ...
        if (declaration.kind === 26 /* Parameter */) {
    ...
    function getDeclaredTypeOfTypeParameter(symbol) {
        ...
        if (!links.declaredType) {
            ...
            type.symbol = symbol;
            if (!getDeclarationOfKind(symbol, 27 /* TypeParameter */).constraint) {
    ...
    function getConstraintOfTypeParameter(type) {
        if (!type.constraint) {
            if (type.target) {
                ...
                type.constraint = getTypeFromTypeNode(getDeclarationOfKind(type.symbol, 27 /* TypeParameter */).constraint);
            }
        }
                    ...
                    if (symbol) {
                        ...
                        if (type.flags & (1024 /* Class */ | 2048 /* Interface */) && type.flags & 4096 /* Reference */) {
                            ...
                            if (node.arguments && node.arguments.length === typeParameters.length) {
                                type = createTypeReference(type, reflect.map(node.arguments, function (t) {
    ...
    function checkTypeSubtypeOf(source, target) {
    ...
    function checkTypeAssignableTo(source, target) {
    ...
    function checkTypeRelatedTo(source, target, relation) {
        var errorInfo;
        var sourceStack;
        var targetStack;
        var expandingFlags;
        ...
        function reportError(message, arg0, arg1) {
            errorInfo = reflect.DiagnosticMessageChain.chain(errorInfo, message, arg0, arg1);
        }
            ...
            for (var i = 0; i < properties.length; i++) {
                ...
                if (!sourceProp) {
                    if (!targetPropIsOptional) {
                        if (reportErrors) {
                            reportError(reflect.Diagnostics.Property_0_is_missing_in_type_1, symbolToString(targetProp), typeToString(source));
                        }
                        return false;
                    }
                    ...
                    if (getDeclarationFlagsFromSymbol(sourceProp) & 32 /* Private */ || getDeclarationFlagsFromSymbol(targetProp) & 32 /* Private */) {
                        if (sourceProp.valueDeclaration !== targetProp.valueDeclaration) {
                            if (reportErrors) {
                                reportError(reflect.Diagnostics.Private_property_0_cannot_be_reimplemented, symbolToString(targetProp));
                            }
                            return false;
                        }
                    }
                    if (!isRelatedTo(getTypeOfSymbol(sourceProp), getTypeOfSymbol(targetProp), reportErrors)) {
                        ...
                        if (reportErrors) {
                            reportError(reflect.Diagnostics.Required_property_0_cannot_be_reimplemented_with_optional_property_in_1, targetProp.name, typeToString(source));
                        }
                        return false;
                    }
                }
        ...
        function stringIndexTypesRelatedTo(source, target, reportErrors) {
            if (relation === identityRelation) {
                ...
                if (targetType) {
                    ...
                    if (!sourceType) {
                        if (reportErrors) {
                            reportError(reflect.Diagnostics.Index_signature_is_missing_in_type_0, typeToString(source));
                        }
                        return false;
                    }
...
(function (reflect) {
    ...
    var files = reflect.processRootFile("tests/tsreflect.d.json");

    reflect.forEach(files, reflect.bindSourceFile);
    reflect.initializeGlobalTypes();

    var GenericType = reflect.getDeclaredTypeOfSymbol(reflect.resolveEntityName(undefined, "reflect.GenericType", reflect.SymbolFlags.Type));
    var Type = reflect.getDeclaredTypeOfSymbol(reflect.resolveEntityName(undefined, "reflect.Type", reflect.SymbolFlags.Type));

    console.log(reflect.isTypeAssignableTo(Type, GenericType));
    console.log(reflect.isTypeAssignableTo(GenericType, Type));

    reflect.printTypeErrors();
