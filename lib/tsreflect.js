...
(function (reflect) {
    ...
    function removeFileExtension(path, ext) {
        if (ext) {
...
(function (reflect) {
    function createLoader() {
        ...
        function getLoadedSourceFile(filename) {
            filename = reflect.normalizePath(reflect.getCanonicalFileName(filename));
        ...
        function processExternalModule(moduleName, searchPath) {
            ...
            if (!isRelative) {
                var symbol = checker.getSymbol(checker.getGlobals(), '"' + moduleName + '"', 512 /* ValueModule */);
                if (symbol) {
        ...
        function processReferencedFilesAsync(file, basePath, callback) {
            async.eachSeries(file.references || [], function (filename, callback) {
        ...
        function processImportedModulesAsync(file, basePath, callback) {
            async.eachSeries(getExternalImportDeclarations(file), function (found, callback) {
                var node = found.node;
                if (!found.topLevel) {
        ...
        function processImportedModules(file, basePath) {
            reflect.forEach(getExternalImportDeclarations(file), function (found) {
                var node = found.node;
                if (found.topLevel) {
        ...
        function getExternalImportDeclarations(file) {
            ...
            reflect.forEach(file.declares, function (node) {
                if (node.kind === 7 /* ImportDeclaration */ && node.require) {
                    var moduleName = node.require;
                    if (moduleName) {
                        nodes.push({ topLevel: true, node: node });
                    }
                }
...
(function (reflect) {
    var SymbolImpl = (function () {
        ...
        SymbolImpl.prototype.invoke = function (obj, args) {
            if (!this.name || (this.flags & 8192 /* Method */) === 0) {
...
(function (reflect) {
    var TypeImpl = (function () {
        ...
        TypeImpl.prototype.hasInterface = function (target) {
            // TODO: cache?
            if (this.isClass()) {
                var type = this;
                while (type) {
        ...
        TypeImpl.prototype.getInterfaces = function () {
            var ret = [];
            if (this.isClass()) {
                var type = this;
                while (type) {
        ...
        TypeImpl.prototype.getInterface = function (name) {
            // TODO: cache?
            if (this.isClass()) {
                var type = this;
                while (type) {
        ...
        TypeImpl.prototype._getImplementation = function () {
            ...
            if (!moduleSymbol) {
                ...
                if (moduleSymbol.valueDeclaration.kind == 0 /* SourceFile */) {
                    if (!reflect.isRelativePath(moduleName) && !reflect.isAbsolutePath(moduleName)) {
...
(function (reflect) {
    ...
    function createTypeChecker(loader) {
        ...
        function createIntrinsicType(kind, intrinsicName) {
            ...
            type.symbol = globals[intrinsicName] = createSymbol(3152352 /* Type */, intrinsicName);
            return type;
        }
        ...
        function hasInterface(type, checkBase) {
            return check(type);
            function check(type) {
                var target = getTargetType(type);
                return target === checkBase || reflect.forEach(target.implementedTypes, check);
            }
        }
                ...
                if (declaration.implements) {
                    ...
                    reflect.forEach(declaration.implements, function (node) {
                        ...
                        if (implementedType !== unknownType) {
                            if (getTargetType(implementedType).flags & (1024 /* Class */ | 2048 /* Interface */)) {
                                if (type !== implementedType && !hasInterface(implementedType, type)) {
...
(function (reflect) {
    ...
    function createContext() {
        ...
        function load(paths, callback) {
            ...
            var symbols = [];
            async.eachSeries(paths, processPath, function (err) {
                if (err)
                    return callback(err, null);
            ...
            function processPath(filePath, callback) {
                ...
                glob(relativePath, function (err, matches) {
                    ...
                    async.eachSeries(matches, processFile, function (err) {
                        if (err)
                            return callback(err);
                        callback();
                    });
        ...
        function loadSync(paths) {
            ...
            var symbols = [];
            reflect.forEach(paths, processPath);
            throwIfErrors();
            return symbols;
            function processPath(filePath) {
                ...
                reflect.forEach(matches, processFile);
            }
            function processFile(filePath) {
                getSymbolsFromSourceFile(loader.processRootFile(filePath), symbols);
            }
        }
        function getSymbolsFromSourceFile(sourceFile, symbols) {
            if (!sourceFile)
                return;
        ...
        function checkSymbolForConstructor(symbol, ctr) {
            if (symbol.name === ctr.name && (symbol.flags & 32 /* Class */) != 0) {
                var declaredType = checker.getDeclaredTypeOfSymbol(symbol);
        ...
        function checkSymbolTableForConstructor(table, ctr) {
        ...
        function findAllModulePaths() {
            var filenames = [];
            findDescendantModulePaths(findTopModule(), filenames);
            return filenames;
        }
        function findDescendantModulePaths(node, filenames) {
            var filename = reflect.removeFileExtension(node.filename, ".js") + ".d.json";
            if (fs.existsSync(filename)) {
        ...
        function findTopModule() {
            var parent = module, top;
            while (parent) {
                top = parent;
