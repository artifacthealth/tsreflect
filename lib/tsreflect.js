...
(function (reflect) {
    ...
    function loadDeclarationFile(filePath, callback) {
        reflect.processRootFileAsync(path.relative(process.cwd(), filePath), function (err, sourceFile) {
            if (err)
                return callback(err, null);
            // TODO: how to capture diagnostic errors async? maybe use continuation local storage? maybe just throw errors instead of collecting?
