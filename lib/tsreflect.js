...
(function (reflect) {
    ...
    var Symbol = (function () {
        ...
        Symbol.prototype.resolve = function (name, meaning) {
            if (typeof meaning === "undefined") { meaning = SymbolFlags.Namespace | SymbolFlags.Type | SymbolFlags.Value; }
            if (!name) {
            ...
            return reflect.resolveEntityName(this.declarations[0], name, meaning);
        };
        ...
        Symbol.prototype.getDeclaredType = function () {
            return reflect.getDeclaredTypeOfSymbol(this);
        };
        return Symbol;
    })();
...
(function (reflect) {
    ...
    function getDefaultLibFilename() {
        return reflect.combinePaths(reflect.normalizePath(__dirname), "lib.d.json");
    }
...
(function (reflect) {
    var Diagnostic = (function () {
        ...
        Diagnostic.chain = function (next, message) {
            var text = getLocaleSpecificMessage(message.key);
...
(function (reflect) {
    ...
    function checkTypeRelatedTo(source, target, relation) {
        ...
        function reportError(message, arg0, arg1, arg2) {
            errorInfo = reflect.Diagnostic.chain(errorInfo, message, arg0, arg1, arg2);
        }
                    ...
                    if (!sourceProp) {
                        ...
                        if (sourceFlags & 32 /* Private */ || targetFlags & 32 /* Private */) {
                            if (sourceProp.valueDeclaration !== targetProp.valueDeclaration) {
                                if (reportErrors) {
                                    if (sourceFlags & 32 /* Private */ && targetFlags & 32 /* Private */) {
                                        ...
                                        reportError(reflect.Diagnostics.Property_0_is_private_in_type_1_but_not_in_type_2, symbolToString(targetProp), typeToString(sourceFlags & 32 /* Private */ ? source : target), typeToString(sourceFlags & 32 /* Private */ ? target : source));
                                    }
                                }
                                return false;
                            }
                            ...
                            if (!sourceClass || !hasBaseType(sourceClass, targetClass)) {
                                if (reportErrors) {
                                    reportError(reflect.Diagnostics.Property_0_is_protected_but_type_1_is_not_a_class_derived_from_2, symbolToString(targetProp), typeToString(sourceClass || source), typeToString(targetClass));
                                }
                                return false;
                            }
                        } else if (sourceFlags & 64 /* Protected */) {
                            if (reportErrors) {
                                reportError(reflect.Diagnostics.Property_0_is_protected_in_type_1_but_public_in_type_2, symbolToString(targetProp), typeToString(source), typeToString(target));
                            }
                            return false;
                        }
                        ...
                        if (isOptionalProperty(sourceProp) && !isOptionalProperty(targetProp)) {
                            if (reportErrors) {
                                reportError(reflect.Diagnostics.Property_0_is_optional_in_type_1_but_required_in_type_2, symbolToString(targetProp), typeToString(source), typeToString(target));
                            }
                            return false;
                        }
                    }
    ...
    function getBestCommonType(types, contextualType, candidatesOnly) {
        ...
        return reflect.forEach(types, function (t) {
            return isSupertypeOfEach(t, types) ? t : undefined;
        }) || (candidatesOnly ? undefined : emptyObjectType);
    }
...
(function (reflect) {
    ...
    function require(moduleName) {
        ...
        while (true) {
            var filename = reflect.normalizePath(reflect.combinePaths(searchPath, moduleName));
            var sourceFile = reflect.processRootFile(filename + ".d.json");
            if (sourceFile || isRelative)
                break;
            var parentPath = reflect.getDirectoryPath(searchPath);
            if (parentPath === searchPath)
    ...
    function resolve(name, meaning) {
        if (typeof meaning === "undefined") { meaning = reflect.SymbolFlags.Namespace | reflect.SymbolFlags.Type | reflect.SymbolFlags.Value; }
        if (!name) {
            throw new Error("Missing required argument 'name'.");
        }

        return reflect.resolveEntityName(undefined, name, meaning);
    }
    exports.resolve = resolve;

    function getBasePath() {
        return reflect.getDirectoryPath(module.parent.filename);
    }
})(reflect || (reflect = {}));
